<?php
  ...
  function regen_password($param, $dbcon, $atoken, &$regenpassword)
  {
    try{
      ...
      // 아이디가 존재하는지 검사
      $sql = "SELECT * FROM aw_member WHERE mem_id = :mem_id";
      $stmt = $dbcon->prepare($sql);
      $stmt->bindValue(":mem_id", $param->userid, PDO::PARAM_STR);
      $stmt->execute();
      ...
      $sql = "UPDATE aw_member SET mem_password = :mem_password WHERE mem_id = :mem_id";
      $stmt = $dbcon->prepare($sql);
