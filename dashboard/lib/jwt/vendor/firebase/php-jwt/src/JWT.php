...
class JWT
{
    ...
    public static function decode($jwt, $key, array $allowed_algs = array())
    {
        ...
        $tks = \explode('.', $jwt);
        if (\count($tks) != 3) {
            //throw new UnexpectedValueException(401);
            throw new Exception(401);
        }
        list($headb64, $bodyb64, $cryptob64) = $tks;
        if (null === ($header = static::jsonDecode(static::urlsafeB64Decode($headb64)))) {
            throw new Exception(402);
            //throw new UnexpectedValueException(402);
        }
        if (null === $payload = static::jsonDecode(static::urlsafeB64Decode($bodyb64))) {
            throw new Exception(403);
            //throw new UnexpectedValueException(403);
        }
        ...
        if (empty($header->alg)) {
            ...
            //throw new UnexpectedValueException(405);
        }
        if (empty(static::$supported_algs[$header->alg])) {
            throw new Exception(406);
            //throw new UnexpectedValueException(406);
        }
        if (!\in_array($header->alg, $allowed_algs)) {
            throw new Exception(407);
            //throw new UnexpectedValueException(407);
        }
        ...
        if (\is_array($key) || $key instanceof \ArrayAccess) {
            if (isset($header->kid)) {
                if (!isset($key[$header->kid])) {
                    ...
                    //throw new UnexpectedValueException(408);
                }
                $key = $key[$header->kid];
            } else {
                throw new Exception(409);
                //throw new UnexpectedValueException(409);
            }
        }
